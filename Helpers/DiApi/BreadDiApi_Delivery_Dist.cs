...
{
    public class BreadDiApi_Delivery_Dist
    {
        ...
        string GrRef2 { get; set; }
        string GrRemarks { get; set; }
        ...
        string PostedDocEntry { get; set; }
        string PostedDocNum { get; set; }

        // constructor 
        public BreadDiApi_Delivery_Dist(string crCommDbConnStr)
        {            
            Conn = new SqlConnection(crCommDbConnStr);
            GrRemarks = $@"GOODS RECEIPT FROM PRODUCTION {DateTime.Now:dd-MMM-yyyy} (GR-IN)";
        }

        // entry point 
        public string CreateGoodsReceiveInvoice_Dist(Bread_OINV_Ext invDoc,
                                                    List<Bread_INV1_Ext> lines)
        {
            string modName = $"[BreadDiApi_Delivery_Dist][CreateGoodsReceiveInvoice_Dist]";
            try
            {
                // query the inv 
                ...
                // for later remark, and invoice update
                ...
                grDoc1.CardCode = cardCode;

                if (!string.IsNullOrWhiteSpace(invDoc.REMARKS))
                {
                    grDoc.JournalMemo = $"{invDoc.REMARKS}";
                }

                if (!string.IsNullOrWhiteSpace(GrRemarks))
                {
                    grDoc.UserFields.Fields.Item("U_CSUS_REMARKS").Value = GrRemarks;
                    grDoc.Comments = $"{GrRemarks}"; // 20220313 use auto remark from api
                }

                // cross reference
                if (!string.IsNullOrWhiteSpace(GrRef2))
                {
                    grDoc.Reference2 = GrRef2;
                }
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    double price0 = GetGrPrice(Db.SAPDB, itemCode.ItemCode);
                    if (price0 > 0)
                    {
                        grDoc.Lines.Price = price0;
                    }

                    if (!string.IsNullOrWhiteSpace(cardBranch))  // dim 1
                    {
                        grDoc.Lines.CostingCode = cardBranch; // dim 1
                    }
                    ...
                    {
                        // get all it line 
                        var batches = lines.Where(i => i.ITEMCODE.Equals(itemCode.ItemCode)).ToList();
                        //var batLineCount = 0;
                        for (int y = 0; y < batches.Count; y++)
                        {
                            ...
                            grDoc.Lines.BatchNumbers.Quantity = (double) batch.QUANTITY;
                ...
                GrRef2 = "";
                InvRef2 = $"{grDoc1.DocNum}";
                InvRemarks = $"GR - IN # {grDoc1.DocNum}, {DateTime.Now:dd-MMM-yyyy}";
                #endregion
                // ---------------------------------------------------------------------------------
                ...
                {   
                    ...
                    if (oInv1.GetByKey(int.Parse(invEntry)))
                    {
                        ...
                        GrRef2 = $"{oInv1.DocNum}";     // for update the gr ref2                     
                    }
                    else
                    {
                        oInv1 = null;
